...
You can control whether the turtle's movement will leave a trace using `PenUp()` and `PenDown()`, and control its movement using `Forward()`, `Turn()`, and `GoTo()`. Finally, `GetX()` and `GetY()` tell you the current center of the turtle.
...
If you are lucky, the mocks you need to use have already been implemented by some nice people. If, however, you find yourself in the center to write a mock class, relax - Google Mock turns this task into a fun game! (Well, almost.)
